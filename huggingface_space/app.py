...
# Class names
DEFECT_CLASSES = [
    "çizik", "çatlak", "delik", "ezilme", 
    "yanık", "pas", "diğer"
]

ORE_CLASSES = [
    "manyetit", "kromit", "pirit", "kalkopirit", 
    "atık", "düşük tenörlü"
]
    ...
    try:
        ...
        if result.boxes is not None and len(result.boxes) > 0:
            ...
            for box, conf, cls in zip(boxes, confs, classes):
                cls_idx = int(cls)
                cls_name = DEFECT_CLASSES[cls_idx] if cls_idx < len(DEFECT_CLASSES) else f"sınıf_{cls_idx}"
                detections.append({
                    "class": cls_name,
...
def run_ore_classification(image: Image.Image, confidence: float = 0.25):
    ...
    try:
        ...
        if result.boxes is not None and len(result.boxes) > 0:
            ...
            for box, conf, cls in zip(boxes, confs, classes):
                cls_idx = int(cls)
                cls_name = ORE_CLASSES[cls_idx] if cls_idx < len(ORE_CLASSES) else f"sınıf_{cls_idx}"
                detections.append({
                    "class": cls_name,
                    "confidence": float(conf),
                    "bbox": [float(x) for x in box]
