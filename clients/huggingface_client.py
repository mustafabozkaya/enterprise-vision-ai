...
class SAMModel:
    ...
    def predict(
        ...
        if len(boxes) == 0:
            return VisionResult(
                boxes=np.array([]).reshape(0, 4), labels=[], scores=np.array([]), masks=None
            )
    ...
    def generate_masks(
        self, image: Union[str, Image.Image, np.ndarray], num_points: int = 64
    ) -> VisionResult:
        """
        ...
        return self.predict(image, np.array(points), np.array(labels), multimask_output=False)
...
class HuggingFaceVisionClient:
    ...
    # Model türleri
    MODEL_TYPES = {
        "sam": {"default": "sam-vit-base", "class": SAMModel},
        "detection": {"default": "detr-resnet-50", "class": DetectionModel},
    }

    def __init__(self, device: str = "cpu", cache_dir: str = "models/huggingface"):
        """
    ...
    def load_model(
        self, model_type: str, model_name: Optional[str] = None, force: bool = False
    ) -> bool:
        """
        Modeli yükler.
        ...
        model = model_class(model_id=model_name, device=self.device, cache_dir=self.cache_dir)
    ...
    def segment_and_detect(
        ...
        for box in detections.boxes:
            ...
            sam_result = sam_model.predict(
                image, points=np.array([[center_x, center_y]]), labels=np.array([1])
            )
...
class HuggingFaceClient:
    ...
    def __init__(self, api_token: Optional[str] = None, cache_dir: str = "models/huggingface"):
        """
    ...
    def list_models(
        self, filter: Optional[str] = None, search: Optional[str] = None, limit: int = 10
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            models = list_models(filter=filter, search=search, limit=limit, token=self.api_token)
    ...
    def download_model(self, model_id: str, revision: Optional[str] = None) -> str:
        """
        Modeli indirir.
        ...
        try:
            ...
            model_path = snapshot_download(
                repo_id=model_id, revision=revision, cache_dir=self.cache_dir, token=self.api_token
            )
    ...
    def upload_model(
        self, model_path: str, repo_id: str, commit_message: str = "Upload model"
    ) -> str:
        """
        Modeli yükler.
...
class HuggingFaceInferenceClient:
    ...
    def __init__(self, api_token: Optional[str] = None, model_id: str = "facebook/detr-resnet-50"):
        """
    ...
    def classify_image(
        self, image: Union[str, Image.Image, bytes], candidate_labels: List[str]
    ) -> List[Dict[str, Any]]:
        """
    ...
    def detect_objects(self, image: Union[str, Image.Image, bytes]) -> List[Dict[str, Any]]:
        """
        Nesne tespiti yapar.
    ...
    def segment_image(self, image: Union[str, Image.Image, bytes]) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            return [
                {"label": item.label, "score": item.score, "mask": item.mask}  # Base64 encoded
                for item in result
            ]
...
class HuggingFaceSpacesClient:
    ...
    def __init__(self, space_id: str, api_token: Optional[str] = None):
        """
    ...
    def predict(self, data: Dict[str, Any], timeout: int = 60) -> Dict[str, Any]:
        """
