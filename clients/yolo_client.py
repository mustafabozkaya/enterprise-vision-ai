...
class YOLOModelManager:
    ...
    # Varsayılan modeller
    DEFAULT_MODELS = {"detection": "yolo11n.pt", "segmentation": "yolo11n-seg.pt"}
    ...
    async def load_model(self, model_path: str, device: str = "cpu", verbose: bool = False) -> bool:
        """
        Modeli yükler.
    ...
    async def get_model(self, model_path: str, device: str = "cpu") -> Any:
        """
    ...
    def get_loaded_models(self) -> Dict[str, float]:
        ...
        """
        return {name: self._model_timestamps.get(name, 0) for name in self._models.keys()}
...
class YOLOClient:
    ...
    def predict(
        ...
        **kwargs,
        ...
        """
        return self.model.predict(source=source, conf=conf, iou=iou, max_det=max_det, **kwargs)

    def predict_batch(self, sources: list, conf: float = 0.25, iou: float = 0.45, **kwargs) -> Any:
        """
        Batch inference yap.
        ...
        Returns:
            Any: Inference sonuçları
        """
        return self.model.predict(source=sources, conf=conf, iou=iou, **kwargs)

    def predict_video(
        self, source: str, conf: float = 0.25, iou: float = 0.45, save: bool = False, **kwargs
    ) -> Any:
        """
        Video inference yap.
        ...
        return self.model.predict(
            source=source, conf=conf, iou=iou, save=save, stream=True, **kwargs
        )
