...
@router.get("/upload/files", summary="List Uploaded Files", description="List all uploaded files")
@limiter.limit("30/minute")
async def list_uploaded_files(request: Request, subdir: Optional[str] = None):
    """
    ...
    try:
        ...
        return {"files": files, "count": len(files), "directory": str(upload_path)}
...
@router.delete(
    "/upload/files", summary="Delete Uploaded File", description="Delete a specific uploaded file"
)
@limiter.limit("30/minute")
async def delete_uploaded_file(request: Request, file_path: str):
    """
    ...
    try:
        ...
        if not str(full_path).startswith(str(UPLOAD_DIR.resolve())):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Geçersiz dosya yolu."
            )
        ...
        return {"success": True, "message": f"{full_path.name} dosyası silindi."}
