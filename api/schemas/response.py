...
class DetectionResult(BaseModel):
    ...
    bbox: BoundingBox = Field(..., description="Bounding box koordinatları")
    class_id: int = Field(..., description="Sınıf ID'si")
    mask: Optional[List[List[int]]] = Field(
        default=None, description="Segmentasyon maskesi (polygon koordinatları)"
    )
    ...
    metadata: Optional[Dict[str, Any]] = Field(default=None, description="Ek meta veriler")
...
class ClassificationResult(BaseModel):
    ...
    confidence: float = Field(..., ge=0.0, le=1.0, description="Güven skoru")
    class_id: int = Field(..., description="Sınıf ID'si")
    probability: Optional[float] = Field(default=None, description="Olasılık değeri")
...
class ImageInfo(BaseModel):
    ...
    channels: Optional[int] = Field(default=3, description="Kanal sayısı")
    format: Optional[str] = Field(default=None, description="Görüntü formatı")
    size_bytes: Optional[int] = Field(default=None, description="Dosya boyutu (bytes)")
...
class Metrics(BaseModel):
    ...
    total_detections: int = Field(default=0, description="Toplam tespit sayısı")
    processing_time: float = Field(..., description="İşleme süresi (saniye)")
    inference_time: Optional[float] = Field(default=None, description="Inference süresi (saniye)")
    metal_ratio: Optional[float] = Field(default=None, description="Metal oranı (%)")
    anomaly_score: Optional[float] = Field(default=None, description="Anomali skoru (0-100)")
    severity_distribution: Optional[Dict[str, int]] = Field(
        default=None, description="Severity dağılımı"
    )
    class_distribution: Optional[Dict[str, int]] = Field(default=None, description="Sınıf dağılımı")
...
class InferenceResponse(BaseModel):
    ...
    task_type: str = Field(
        ..., description="Görev türü", examples=["defect_detection", "ore_classification"]
    )
    results: List[DetectionResult] = Field(default_factory=list, description="Tespit sonuçları")
    processing_time: float = Field(..., description="Toplam işleme süresi (saniye)")
    model_used: str = Field(..., description="Kullanılan model")
    image_info: Dict[str, Any] = Field(default_factory=dict, description="Görüntü bilgileri")
    metrics: Optional[Metrics] = Field(default=None, description="Metrikler")
    message: Optional[str] = Field(default=None, description="Ek mesaj")
    error: Optional[str] = Field(default=None, description="Hata mesajı (başarısız ise)")
...
class BatchUploadResponse(BaseModel):
    ...
    errors: Optional[List[Dict[str, str]]] = Field(default=None, description="Hata listesi")
    message: str = Field(..., description="Mesaj")
    processing_time: float = Field(..., description="İşleme süresi")
...
class ModelInfo(BaseModel):
    ...
    classes: Optional[List[str]] = Field(default=None, description="Sınıf listesi")
    metadata: Optional[Dict[str, Any]] = Field(default=None, description="Ek meta veriler")
...
class HealthResponse(BaseModel):
    ...
    status: str = Field(
        ..., description="API durumu", examples=["healthy", "degraded", "unhealthy"]
    )
    version: str = Field(..., description="API versiyonu")
    models_loaded: List[str] = Field(default_factory=list, description="Yüklenmiş modeller")
    models_count: int = Field(..., description="Yüklenmiş model sayısı")
    timestamp: datetime = Field(default_factory=datetime.now, description="Zaman damgası")
    uptime_seconds: Optional[float] = Field(default=None, description="Çalışma süresi (saniye)")
