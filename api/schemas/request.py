...
class InferenceRequest(BaseModel):
    ...
    image: str = Field(
        ...
        examples=["base64_encoded_image_string", "https://example.com/image.jpg", "/path/to/image.jpg"]
    )
    image_source: ImageSource = Field(
        default=ImageSource.BASE64,
        description="Görüntü kaynağı türü"
    )
    ...
    iou_threshold: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="IoU eşiği (Non-Maximum Suppression)"
    )
    max_detections: int = Field(
        default=100,
        ge=1,
        le=300,
        description="Maksimum tespit sayısı"
    )
...
class DefectDetectionRequest(InferenceRequest):
    ...
    task_type: TaskType = Field(
        default=TaskType.DEFECT_DETECTION,
        description="Görev türü"
    )
    include_severity: bool = Field(
        default=True,
        description="Severity seviyesini dahil et"
    )
...
class OreClassificationRequest(InferenceRequest):
    ...
    """
    task_type: TaskType = Field(
        default=TaskType.ORE_CLASSIFICATION,
        description="Görev türü"
    )
...
class UploadRequest(BaseModel):
    ...
    subdir: str = Field(
        default="images",
        description="Kaydedilecek alt dizin"
    )
...
class BatchUploadRequest(BaseModel):
    ...
    wait_for_processing: bool = Field(
        default=False,
        description="Yükleme sonrası işleme bekle"
    )
...
class ModelListRequest(BaseModel):
    ...
    include_loaded_only: bool = Field(
        default=False,
        description="Sadece yüklenmiş modelleri dahil et"
    )
    model_type: Optional[str] = Field(
        default=None,
        description="Model türü filtresi"
    )
...
class HealthCheckRequest(BaseModel):
    ...
    include_models: bool = Field(
        default=True,
        description="Model durumlarını dahil et"
    )
    include_system: bool = Field(
        default=True,
        description="Sistem bilgilerini dahil et"
    )
