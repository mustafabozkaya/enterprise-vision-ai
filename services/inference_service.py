        ...
        if effective_provider == ModelProvider.HUGGINGFACE:
            ...
            try:
                ...
                try:
                    ...
                    raise RuntimeError(
                        f"Hem YOLO hem de HuggingFace başarısız: {hf_error}"
                    )
    ...
    async def _detect_defects_huggingface(
        ...
        try:
            ...
            metrics = Metrics(
                ...
                processing_time=0.0,
                anomaly_score=float(np.mean(vision_result.scores) * 100) if len(vision_result.scores) > 0 else 0.0,
    ...
    async def _classify_ore_huggingface(
        ...
        try:
            ...
            metrics = Metrics(
                total_detections=len(detections),
                processing_time=0.0,
                metal_ratio=float(sum(
                    1 for l in labels if l in ["manyetit", "krom"]
                ) / max(len(labels), 1)),
